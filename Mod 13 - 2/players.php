    ...
    <script>
        ...
        setTimeout(() => {
            fetch("bot_page.php")
                ...
                .then(html => {
                    const container = document.getElementById("chat-wrapper");
                    container.innerHTML = html;

                    // ⬇ re-run the bot JS manually
                    const script = container.querySelector("script");
                    if (script) {
                        eval(script.innerText);
                    }
                })
                .catch(err => console.log("Error loading chatbot:", err));
        }, 10000);
    </script>

    <style>
        /*chatbot style on players side*/
        #chat-wrapper {
            position: fixed;
            bottom: 160px;
            right: 210px;
...
<?php
    ...
    logActivity($_SESSION['username'],"Load Player Page", "Player Page Loaded");
